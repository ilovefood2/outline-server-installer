...
```bash
sudo bash -c "$(wget -qO- https://raw.githubusercontent.com/ilovefood2/outline-server-installer/v1.12.3-r5/ec2/bootstrap.sh)"
```

It downloads the tagged source to `/opt/outline-server-installer-v1.12.3-r5`, then starts the EC2 installer. The public IP is auto-detected; arguments after `--` are forwarded to `ec2/install.sh`, including `--rebuild` for an in-place image upgrade.
