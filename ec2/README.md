...
```bash
sudo bash -c "$(wget -qO- https://raw.githubusercontent.com/ilovefood2/outline-server-installer/v1.12.3-r4/ec2/bootstrap.sh)" -- --hostname YOUR_ELASTIC_IP_OR_DNS
```

It downloads the tagged source to `/opt/outline-server-installer-v1.12.3-r4`, then starts the EC2 installer. Arguments after `--` are forwarded to `ec2/install.sh`.
