...
grep -Fq 'prepare_build_workspace' "${ROOT_DIR}/install.sh"
grep -Fq 'FORCE_BUILD=0' "${ROOT_DIR}/install.sh"
grep -Fq 'IMAGE_VERSION="${SERVER_VERSION}"' "${ROOT_DIR}/scripts/build_image.sh"
...
bash "${ROOT_DIR}/ec2/bootstrap.sh" --help | grep -Fq 'Outline Server EC2 bootstrap (v1.12.3-r5)'
